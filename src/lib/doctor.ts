...
import { getPlatformPaths, getPlatformInfo } from './platform-utils.js';
import { IPFSClientManager } from './ipfs-client-manager.js';
 ...
 */
export async function runDoctor(options: { global?: boolean; verbose?: boolean; json?: boolean }): Promise<void> {
  try {
...
async function checkEnvFile(verbose?: boolean, baseDir?: string): Promise<HealthCheck> {
  try {
    ...
    return {
      ...
      status: 'fail',
      message: 'Not found',
      details: 'Run "lsh sync --init" to create configuration',
    };
  }
}
  ...
  try {
    ...
    if (!match) {
      return {
        ...
        details: 'Run "lsh sync --key" to generate a key',
      };
    }
    ...
    if (key.length < 32) {
      return {
        ...
        details: 'Generate a stronger key with "lsh sync --key"',
      };
    }
  ...
  try {
    ...
    if (storageMode === 'local') {
      ...
      checks.push({
        ...
        details: 'Use "lsh push" to sync secrets',
      });
    } else {
      checks.push({
        name: 'Storage Backend',
...
async function checkIPFSClient(verbose?: boolean): Promise<HealthCheck> {
  try {
    ...
    return {
      ...
      details: 'Install with: lsh sync --init',
    };
  } catch (error) {
    const err = error as Error;
    return {
...
function displayRecommendations(checks: HealthCheck[]): void {
  ...
  if (!hasIssues) {
    ...
    if (checks.some(c => c.name === '.env File' && c.status === 'fail')) {
      console.log(chalk.cyan('👉 Run: lsh sync --init'));
    }
