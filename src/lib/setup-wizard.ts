...
async function runInteractiveSetup(options: {
  ...
  if (existingConfig) {
    ...
    if (!overwrite) {
      ...
      console.log(chalk.gray('\nRun'), chalk.cyan('lsh sync --doctor'), chalk.gray('to check your current setup.'));
      return;
    }
  }

  // Check IPFS daemon status
  const { getIPFSSync } = await import('./ipfs-sync.js');
...
function showSuccessMessage(config: InitConfig): void {
  ...
  console.log(chalk.cyan('      lsh sync --init # Choose "Import existing key"'));
